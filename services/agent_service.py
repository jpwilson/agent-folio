...
async def _get_provider(user_id: str, fallback_token: str):
    ...
    try:
        ...
        logger.info(
            "_get_provider: found %d active backends for user %s", len(connections) if connections else 0, user_id
        )
        if connections:
            ...
            for c in connections:
                try:
                    ...
                    logger.error(
                        "Failed to build %s provider (%s): %s", c.get("provider"), c.get("base_url"), e, exc_info=True
                    )
